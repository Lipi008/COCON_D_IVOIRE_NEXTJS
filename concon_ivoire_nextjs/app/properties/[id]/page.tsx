...
function PropertyDetailsPage({ id }: { id: string }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        {images.slice(1, 5).map((image, index) => (
          <PropertyGalleryThumbnail key={index} image={image} index={index} />
        ))}
      </div>
    </div>
  );
...
function PropertyGalleryThumbnail({
  image,
  index,
}: {
  image: string;
...
// ... continuer avec les autres composants (PropertyTabs, AgentCard, SimilarProperties, etc.) ...

// Supprimer le deuxième export default
