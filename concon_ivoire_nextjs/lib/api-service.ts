...
const API_CONFIG = {
  BASE_URL:
    process.env.NEXT_PUBLIC_API_URL || "https://api.quarter-realestate.com/api",
...
class ApiService {
  ...
  private getMockData<T>(endpoint: string): Promise<ApiResponse<T>> {
    ...
    if (endpoint.startsWith("/properties")) {
      // Cas où on demande une propriété spécifique
      const id = endpoint.match(/\/properties\/(\d+)/)?.[1];
      if (id) {
        const property = mockProperties.find((p) => p.id.toString() === id);
      ...
      // Pagination - utilise les valeurs par défaut si non spécifiées
      const page = Number(filters.page) || 1;
      const limit = Number(filters.limit) || 10;
      const startIndex = (page - 1) * limit;
      const endIndex = startIndex + limit;
