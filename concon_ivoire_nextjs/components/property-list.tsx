...
export default function PropertyList({
  ...
  const { data, isLoading, error, isUsingMock, refetch } = useProperties(currentFilters)
        ...
        {isLoading ? (
          ...
          data.data.map((property) => (
            <Card key={property.id} className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
              <div className="relative">
                ...
                <Badge className="absolute top-4 left-4 bg-primary-DEFAULT">{property.status}</Badge>
              </div>
          ...
          <div className="col-span-full text-center py-12">
            ...
            <p className="text-gray-500">
              Essayez de modifier vos critères de recherche pour trouver des propriétés correspondantes.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="flex space-x-2">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <Button
                key={page}
                variant="outline"
                className={`w-10 h-10 p-0 ${page === currentPage ? "bg-primary-DEFAULT text-white" : ""}`}
